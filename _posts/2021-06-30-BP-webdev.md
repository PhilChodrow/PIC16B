...
Personally, I found it helpful to put navigation links (the top two links at the top of the screen) inside a template called `base.html`. I then had the `submit.html` template `extend` `base.html`. 
...
<figure class="image" style="width:30%">
    ...
    <figcaption><i>Example of the message submission interface. Yours can look different!</i></figcaption>
</figure>
...
4. The post includes the two required screencaps demonstrating the app. 
